<template>
  <div>
    ...
    <div v-if="createControl===true">
      ...
      <input type="text" class="form-control" placeholder="ชื่อประเภท" v-model="datas.type">
...
export default {
  ...
  methods: {
    ...
    validation(){
      if(this.datas.type=''){
        this.alert.text='กรุณากรอกข้อมูsล'
    ...
    submitCreate(){
      if (this.validation()) {
            axios.post(`${DB_URL}/stock/type.json`,this.datas.type).then((result) => {
