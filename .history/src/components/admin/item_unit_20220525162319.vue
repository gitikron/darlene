...
{{alert2.text}},คลิ๊กเพื่อปิดการแจ้งเตือน.
    ...
    <label for="unit" class="card p-2 mb-2" v-if="unitControl===true"> หน่วย
      ...
      <table class="table table-hover mt-2" id="datatable" cellspacing="0" width="100%">
        <thead>
          <tr class="px-0">
            ...
            <th class="col-1 fs-res mx-0 px-0 text-center">
...
export default {
  ...
  methods: {
    ...
    validationEDIT(){
      if (this.edit.arr=='') {
        this.alert3.text='กรุณากรอกหน่วย'
    ...
    saveEdit(){
      if (this.validationEDIT()) {
        axios.patch(`${DB_URL}/items/unit/${this.edit.index}.json`,{units:this.edit.arr})
        .then((result) => {
          this.alert4.text='แก้ไขสำเร็จ'
