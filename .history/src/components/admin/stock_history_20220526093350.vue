      ...
      >
        ...
        <tbody>
          <tr v-for="item,index in arr" :key="index">
            <td class="fs-res mx-0 px-0" v-if="item.action==='delete'"> {{item.who+" ลบ "+item.name+" "}}คงเหลือล่าสุด <span v-if="item.amount===''">0</span><span v-if="item.amount!=''">0</span> {{" "+item.unit}}</td>
