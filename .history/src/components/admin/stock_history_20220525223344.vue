<template>
  <div>
   ...
   <div class="card" v-for="item,index in arr" :key="index">
     <span v-if="item.action === 'input'" class="text-success">{{item.who+" นำเข้า "+item.name+" จำนวน "+item.amount+" "+item.unit }}</span>
     <span v-if="item.action === 'output'" class="text-bg-primary">{{item.who+" นำออก "+item.name+" จำนวน "+item.amount+" "+item.unit }}</span>
