<template>
  <div>
   ...
   <div class="card mt-2" v-for="item,index in arr" :key="index">
     <span v-if="item.action === 'input'" >{{item.who+" นำเข้า "+item.name+" จำนวน "+item.amount+" "+item.unit+" timestamp : "+item.date }}</span>
     <span v-if="item.action === 'output'" class="text-danger">{{item.who+" นำออก "+item.name+" จำนวน "+item.amount+" "+item.unit+" timestamp : "+item.date }}</span>
     <span v-if="item.action === 'delete'" class="text-danger fw-bold">{{item.who+" ลบ "+item.name+" คงเหลือในคลังล่าสุด "+item.amount+" "+item.unit+" timestamp : "+item.date}}</span>
   </div>
  </div>
</template>
