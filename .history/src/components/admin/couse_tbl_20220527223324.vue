<template>
  <div>
    <div class="d-flex justify-content-end">
      <button class="btn btn-success btn-sm" @click="controllerX()">create</button>
    </div>
      ...
      >
        <thead>
          <tr class="px-0">
            ...
            <th class="fs-res mx-0 px-0">Type</th>
            <th class="fs-res mx-0 px-0 text-center">จำนวน</th>
            <th class="fs-res mx-0 px-0 text-center">ราคา</th>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="alert alert-success" role="alert" v-if="alert2.control===true" @click="alert2.control=false,alert2.text=''">
            ...
            <div>
              <label for="upload">Display (PNG/JPEG/GIF size:100*100px)</label>
            <input accept="image/*" class="mt-10 w-100 form-control" type="file" placeholder="select" @change="onFileSelected2($event)"/>
            </div>
            <div class="mt-2">
              <label for="CouseName">type</label>
              <option value="">เลือกประเภท</option>
  ...
  methods: {
    saveEdit(){
     ...
     ValidateEdit(){
      ...
      if(this.edit.arr.couseName==''){
         alert('กรุณากรอกชื่อตอส')
        return false
      }
      if(this.edit.arr.couseName.length>=27){
         alert('ชื่อตอสยาวเกินไป ไม่ควรเกิน 26 ตัว')
        return false
      }
      ...
      if(this.edit.arr.per<=-1){
        alert('กรุณากรอกจำนวนครั้ง')
        return false
      }
      if(this.edit.arr.therapistComm<=-1){
        alert('กรุณากรอกค่าคือ')
        return false
      }
      if(this.edit.arr.mdHan<=-1){
        alert('กรุณากรอกค่าหมอ')
        return false
      }
      if(this.edit.arr.mdCommission==''){
        alert('กรุณาเลือกว่าใช้คิดค่าคอมหมอหรือไม่')
        return false
      }
      return true
    },
    ...
    Validate(){
      ...
      if(this.create.couseName==''){
        this.alert.text='กรุณากรอกชื่อคอส'
        this.alert.control=true
        return false
      }
      if(this.create.length>=27){
        this.alert.text='ชื่อตอสยาวเกินไป ไม่ควรเกิน 26 ตัว'
        this.alert.control=true
        return false
      }
      if(this.create.type==''){
      ...
      if(this.create.per<=-1){
        this.alert.text='กรุณาใส่จำนวนครั้ง'
        this.alert.control=true
        return false
      }
      ...
      if(this.create.mdCommission<=-1){
        this.alert.text='กรุณาเลือกว่าใช้คิดค่าคอมหมอหรือไม่'
        this.alert.control=true
        return false
      }
      return true
