...
<div>
    ...
    <div class="alert alert-success" role="alert" v-if="alert2.control===true" @click="alert2.control=false,alert2.text=''">
      ...
      <button class="btn btn-success btn-sm w-100 mt-3" @click="CreateOpdCard()" v-if="connect===true&&newOpd.OPDnumber!=''">Create</button>
      <button class="btn btn-dark btn-sm w-100 mt-3" v-if="connect===false&&newOpd.OPDnumber===''" disabled>Create (disabled)</button>
    </form>
  </div>
...
export default {
  ...
  methods: {
    ...
    GetppdCard(){
      ...
      axios.get(`${DB_URL}/OPD.json`).then((result) => {
        if (result.data==null) {
          ...
          const newNo =result.data.length-1
          console.log(newNo)
        }
      }).catch((err) => {
    ...
    CreateOpdCard(){
       axios.patch(`${DB_URL}/OPD/${this.newOpd.OPDnumber}.json`,this.newOpd).then((result) => {
          this.alert2.text='successfully'
          this.alert2.control=true
          this.connect=true
          this.newOpd={OPDnumber:'',firstname:'',lastname:'',nickname:'',bloodGroup:'A',sex:'male',Disease:'-',phone:'',personalID:'',address:''}
          GetppdCard()
       }).catch((err) => {
          this.alert.text='Connecting DB is worng'
          this.alert.control=true
