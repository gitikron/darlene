...
export default {
  ...
  methods: {
    deleteModal(){
      if(confirm('คุณต้องการลบรายการในสต็อก ใช่ หรือ ไม่')){
        ...
        axios.delete(`${DB_URL}/stock/list/${this.useIndex}.json`).then((result) => {
          ...
          const msg = `${this.history.name} ถูกลบโดย ${this.history.who} คงเหลือล่าสุด ${this.history.amount} ${this.history.unit}`
          if(Setting.main==true){
            axios.get(`${command.lineHttp+this.Setting.lineStock}/${msg}`)
          }
          this.getArr2()
    ...
    saveOutput(){
      if (this.outputvalidation()) {
        ...
        axios.patch(`${DB_URL}/stock/list/${this.useIndex}.json`,this.reArr).then((result) => {
          const msg = `${this.history.who} นำ ${this.history.name} ออก จำนวน ${this.input} ${this.history.unit} คงเหลือ ${this.history.amount} ${this.history.unit}`
          if(this.Setting.main==true){
            axios.get(`${command.lineHttp+this.Setting.lineStock}/${msg}`)
          }
          this.input=0
    ...
    saveInput(){
      ...
      axios.patch(`${DB_URL}/stock/list/${this.useIndex}.json`,this.reArr).then((result) => {
        const msg = `${this.history.who} นำเข้า ${this.history.name} จำนวน ${this.input} ${this.history.unit} คงเหลือ ${this.history.amount} ${this.history.unit}`
        axios.get(`${command.lineHttp+command.LINE_STOCK}/${msg}`)
        this.input=0
