      ...
      >
        ...
        <tbody>
          <tr v-for="item,index in arr" :key="index">
            <td class="fs-res mx-0 px-0" v-if="item.action==='delete'"> {{item.who+" ลบ "+item.name+" "}}คงเหลือล่าสุด <span v-if="item.amount===''">0</span> {{" "+item.unit}}</td>
            <td class="fs-res mx-0 px-0" v-if="item.action==='input'"> {{item.who+" นำเข้่า "+item.name+" "}}จำนวน <span v-if="item.amount===''">0</span> {{" "+item.unit}}</td>
          </tr>
        </tbody>
      </table>
      {{arr}}
