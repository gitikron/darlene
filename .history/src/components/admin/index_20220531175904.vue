    ...
    <div class="block-center">
      <div class="card block-1">
        ...
        <div class="float-start" v-if="load==='couse'">
          <div v-for="item,index in selectedA" :key="index" class="btn btn-outline-dark card block-3 m-1 p-1" >
            <span class="position-absolute top-2 ml-2 start-100 translate-middle p-2 bg-success border border-light rounded-circle" v-if="item.select===true"/>
          ...
          <button class="btn btn-sm btn-outline-warning" @click="selected(index)" :disabled="item.select===true">เลือก</button>
          <button class="btn btn-sm btn-outline-danger mt-1" @click="selectedOut(index)" :disabled="item.select===false">ลบ</button>
          </div>
          </div>
...
export default {
  ...
  methods: {
    selected(index){
      this.selectedA[index].select=true

    },
    selectedOut(index){
      this.selectedA[index].select=false
    },
    closeModal(){
    ...
    getCouse(){
        axios.get(`${command.DB_URL}/couse/list.json`).then((result) => {
          ...
          for (let i = 0; i < count; i++) {
            const row = {
            couseName:Object.entries(result.data)[i][1].couseName,
            display:Object.entries(result.data)[i][1].display,
            mdCommission:Object.entries(result.data)[i][1].mdCommission,
            mdHand:Object.entries(result.data)[i][1].mdHand,
            per:Object.entries(result.data)[i][1].per,
            price: Object.entries(result.data)[i][1].price,
            therapistComm: Object.entries(result.data)[i][1].therapistComm,
            type: Object.entries(result.data)[i][1].type,
            select:false
            }
            this.selectedA.splice(1,0,row)
          }
        }).catch((err) => {
          console.log(err)
        });
    ...
    getitem(){
        axios.get(`${command.DB_URL}/items/list.json`).then((result) => {
          ...
          //   // data.splice(1,0,Object.entries(auth)[i])
          // }
        }).catch((err) => {
          console.log(err)
        });
