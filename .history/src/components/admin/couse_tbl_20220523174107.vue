      ...
      >
        <thead>
          <tr class="px-0">
            ...
            <td class="fs-res mx-0 px-0">จำนวน</td>
            <td class="fs-res mx-0 px-0">ราคา</td>
            <th class="fs-res mx-0 px-0">
              <span class="material-symbols-outlined text-warning text-black-50">edit</span>
            </th>
          </tr>
        </thead>
        ...
        <tbody>
          <tr v-for="item,index in arr" :key="index">
            ...
            <td class="fs-res mx-0 px-0">{{ Intl.NumberFormat().format(item.per) }} ครั้ง</td>
            <td class="fs-res mx-0 px-0">{{ Intl.NumberFormat('de-DE', { style: 'currency', currency: 'THB' }).format(item.price) }}</td>
            <td class="fs-res mx-0 px-0">
              <button class="btn btn-outline-warning btn-sm material-symbols-outlined" data-bs-toggle="modal" data-bs-target="#editModal" @click="editModel(index)">edit</button>
            </td>
            <td class="fs-res mx-0 px-0">
              <button class="btn btn-outline-danger btn-sm material-symbols-outlined" @click="deleteModal(index)">delete</button>
            </td>
          </tr>
        </tbody>
      </table>
