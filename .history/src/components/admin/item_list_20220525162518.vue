      ...
      >
        ...
        <tbody>
          <tr v-for="item,index in arr" :key="index">
            ...
            <td class="fs-res mx-0 px-0 text-center">{{item.PricePerUnit+" บาท : 1 "+item.unit}}</td>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          {{alert4.text}},คลิ๊กเพื่อปิดการแจ้งเตือน.
           ...
           <div class="d-flex justify-content-center">
        <img :src="create.display" alt="demoDisplay" class="display-100" v-if="edit.arr.display!=''">
      </div>
      <div>
        <label for="upload">Display (PNG/JPEG/GIF size:100*100px)</label>
      <input accept="image/*" class="mt-10 w-100 form-control" type="file"  @change="onFileSelected($event)"/>
      ...
      <label for="price" class="mt-2">ราคา</label>
      <input type="number" class="form-control mt-2" placeholder="Price" v-model="edit.arr.price" @change="calCu()">

      <label for="amount" class="mt-2">จำนวน</label><span class="text-danger">{{textspan}}</span>
      <input type="number" class="form-control mt-2" placeholder="amount" v-model="edit.arr.amount" @change="calCu()">
        
      <label for="unit" class="mt-2">หน่วย</label>
      <select  class="form-control mt-2" v-model="edit.arr.unit">
        <option value="">เลือก</option>
        <option v-for="item,index in unit" :key="index" :value="item.units">{{edit.arr.units}}</option>
      </select>
