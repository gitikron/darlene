      ...
      >
        ...
        <tbody>
          <tr v-for="item,index in arr" :key="index">
            ...
            <td class="fs-res mx-0 px-0">
              <button class="btn btn-sm material-symbols-outlined text-danger" data-bs-toggle="modal" data-bs-target="#editModal" @click="delType(index)">delete</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <input name="TypeName" type="text" class="col12 form-control" placeholder="Type Name">
        </div>
  ...
  methods: {
    ...
    saveEdit(){
      if(confirm("Press a button!\nEither OK or Cancel.")){
        axios.patch(`${DB_URL}/couse/type/${this.edit.index}.json`,this.edit.type).then((result) => {
          this.alert2.text='Successfuly'
          this.alert2.control=true
          this.GetTable
        }).catch((err) => {
          this.alert.text='ERROR:Somthing went worng'
          this.alert.control=true
          console.log(err)
        });
      }
