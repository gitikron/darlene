        ...
        <tbody>
          <tr v-for="item,index in arr" :key="index">
            ...
            <td class="fs-res mx-0 px-0">
              <select id="position" class="form-control mt-2" v-model="item.role"  @change="Selectdatas(index)">
                <option value="">เลือกสถานะ</option>
                <option value="admin">admin</option>
                <option value="doctor">doctor</option>
                <option value="sale">sale</option>
            ...
            <td class="fs-res mx-0 x-0">
              <select id="position" class="form-control mt-2" v-model="item.position" :disabled="item.role===''">
                <option value="">เลือกรูปแบบ</option>
                <option v-for="item,index in choice" :key="index"  :value="item.value">{{item.value}}</option>
            </select>
            </td>
...
export default {
  ...
  methods: {
    ...
    Selectdatas(index){
      if (this.arr[index].role=='admin') {
        this.choice=[{value:"admin"},{value:"manager"}]
      }
      if (this.arr[index].role=='doctor') {
        this.choice=[{value:"normal"},{value:"CBC commission"}]
      }
      if (this.arr[index].role=='sale') {
        this.choice=[{value:"normal"},{value:"CBC commission"}]
      }
